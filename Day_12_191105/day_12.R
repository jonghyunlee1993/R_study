...
# 표준 데이터 형식이 아니기 때문에 정상적으로 읽어내지 못함.
# 따라서 표준 형태로 인식할 수 있게끔 포맷을 알려줌
as.Date('4월 26, 2018',format='%B %d, %Y')
as.Date('4월 26, 2018',format='%B %d, %y') # %y는 18과 같이 인식함
as.Date('4월 26, 18',format='%B %d, %y')
format(as.Date('4월 26, 2018',format='%B %d, %Y'), "%Y")
format(as.Date('4월 26, 2018',format='%B %d, %Y'), "%A")
...
# 문자열을 날짜+시간형으로
# 일반적으로 월은 소문자 m을 사용하고 분은 대문자 M을 사용한다
...
str(ct) 
str(lt) 
# ct를 쪼개면 vector 가 나오고
# lt를 쪼개면 list가 나온다
# month의 값은 0부터 시작하는 경우가 많음
unclass(ct) 
unclass(lt) 
lt$mon
lt$hour
