...
korpopmap2@data <- korpopmap2@data[-26:-251,] # 서울시 구만 파싱
...
map4<-leaflet(korpopmap2) %>% addTiles() %>% setView(lat=37.559957 ,lng=126.975302 , zoom=11)%>%
  addPolygons(stroke=FALSE,smoothFactor=0.2,fillOpacity=.3, popup=popup1, color=~palette1(crime$살인_발생), group='살인')
# 테두리 그리기
map4<-leaflet(korpopmap2) %>% addTiles() %>% setView(lat=37.559957 ,lng=126.975302 , zoom=11)%>%
...
map8<-leaflet(korpopmap2) %>% addTiles() %>% setView(lat=37.559957 ,lng=126.975302 , zoom=11)%>%
  ...
  addPolygons(stroke=FALSE,smoothFactor=0.2,fillOpacity=.5, popup=popup3, color=~palette3(crime$범죄_발생_총합),group='총 범죄')
map8
# 중복해서 그리는 경우 뒤에 있는 것이 더 상위의 레이어에 그려짐
...
library(ggplot2)
library(plotly)
plot = ggplot(data = iris, aes(x = Petal.Width, y= Petal.Length, col=Species)) + geom_point() 
ggplotly(plot)
...
# 리스트 형식으로 리턴함. 따라서 sapply를 적용하지 않아도 사용 가능
word_data3 <- extractNoun(word_data)
word_data3

add_words <- c("백두산", "남산", "철갑", "가을", "달", "바람", "서리", "삼천리")
